import { getDirectReactMessage, getReactMessage, isBlindedText, normalizeContent, type ChatMessage, type ReactCarrier } from "./message-detector";

declare global {
  interface Window { __chzzkPlusBlindedInstalled?: boolean; }
}

export function installBlindedMessageRestorer(): void {
  "use strict";
  ...
  class BlindedMessageRestorer {
    ...
    private resolveOriginal(item: HTMLElement): string {
      let message = getDirectReactMessage(item) || getReactMessage(item);
      const key = this.findMessageKey(item, message);
      let original = normalizeContent(message?.originalContent ?? message?.content);
      if ((!original || isBlindedText(original)) && key) {
        message = this.getMessageFromChatList(item, key) || message;
        original = normalizeContent(message?.originalContent ?? message?.content);
      }
      if ((!original || isBlindedText(original)) && key) original = this.cache.get(key) || "";
      if (key && original && !isBlindedText(original)) this.remember(key, original);
      return isBlindedText(original) ? "" : original;
    }
    ...
    private processItem(item: HTMLElement): void {
      const message = getDirectReactMessage(item) || getReactMessage(item);
      const key = this.findMessageKey(item, message);
      const directOriginal = normalizeContent(message?.originalContent ?? message?.content);
      if (key && directOriginal && !isBlindedText(directOriginal)) this.remember(key, directOriginal);
      ...
      if (!targets.size) {
        item.querySelectorAll<HTMLElement>(this.textSelector).forEach((text) => {
          if (isBlindedText(text.textContent)) targets.add(text.parentElement || text);
        });
      }
      ...
      for (const target of targets) {
        ...
        if (!textElement || !isBlindedText(textElement.textContent)) continue;
        const original = directOriginal && !isBlindedText(directOriginal)
          ? directOriginal
