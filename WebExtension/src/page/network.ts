(() => {
  ...
  // 광고 주소별 처리표입니다. action이 stub이면 빈 성공 결과로 바꾸고,
  // rewrite이면 원래 결과에서 광고 부분만 지웁니다.
  const routes: AdRoute[] = [
    {
      key: "ad-polling",
      label: "광고 폴링 차단",
      action: "stub",
  ...
  /** 요청 주소와 처리표를 비교해 필요한 광고 처리 방법을 찾습니다. */
  const findRoute = (url: string): AdRoute | null => routes.find((route) => route.match(url)) || null;
  ...
  /**
   * 서버가 보낸 결과에서 광고 일정과 광고 표시 여부만 수정합니다.
   * 실제로 값을 바꿨으면 true를 돌려주어 사용자 알림을 표시할 수 있게 합니다.
   */
  const mutatePayload = (payload: JsonObject, route: AdRoute): boolean => {
